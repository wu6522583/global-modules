...
define(function(require, exports, module){
	...
	require("linkbutton");
	require("pagination");
    require.async("style-module/datagrid.css");
    /**
     * jQuery EasyUI 1.3.5
     * 
    ...
    require("expand_datagrid");
});
define("expand_datagrid",function(require, exports, module){
	$.extend( $.fn.datagrid.defaults.onLoadSuccess__ , function (_d) {});
	$.extend( $.fn.datagrid.defaults.onLoadSuccess ,  function (_d) {
        var _n = $.data(this,"datagrid").options;
        if ( _n["checkData"] ) {
            var _data = _n["checkData"].data;
            var _key = _n["checkData"].field;
            var _len = _data.length;
            var _checkData = {"field":_key};
            for (var i = 0; i<_len;i++) {
                _checkData[_data[i][_key]] = _data[i];
            }
            _n["_checkData"] = _checkData;
            delete _n["checkData"];
        }
        $(this).datagrid('autoSelectItem',_d);
        $.data(this,'datagrid').options.onLoadSuccess__( _d );
    });
	$.extend( $.fn.datagrid.defaults.onCheck__ ,  function () {});
	$.extend( $.fn.datagrid.defaults.onCheck ,  function (_a,_A) {
        $(this).datagrid('addCheckData',[_a,_A]);
        $.data(this,'datagrid').options.onCheck__( _a,_A );
    });
	$.extend( $.fn.datagrid.defaults.onUncheck__ ,  function () {});
	$.extend( $.fn.datagrid.defaults.onUncheck ,  function (_o,_O) {
        $(this).datagrid('delCheckData',[_o,_O]);
        $.data(this,'datagrid').options.onUncheck__( _o,_O );
    });
	$.extend( $.fn.datagrid.methods.autoSelectItem , function ( jq , _d ) {
        var _data = _d.rows;
        var _len = _data.length;
        var _key = this.options(jq)._checkData["field"];
        var _c = this.options(jq)._checkData;
        for (var i = 0; i<_len;i++) {
            if (_c[_data[i][_key]]) {
                this.checkRow(jq,i);
            }
    ...
    $.extend( $.fn.datagrid.methods.addCheckData , function ( jq , _o ) {
        var _c = this.options(jq)._checkData;
        var _key = this.options(jq)._checkData["field"];
        _c[_o[1][_key]] = _o[1];
        return jq;
    } );
    $.extend( $.fn.datagrid.methods.delCheckData , function ( jq , _o ) {
        var _c = this.options(jq)._checkData;
        var _key = this.options(jq)._checkData["field"];
        delete _c[_o[1][_key]];
        return jq;
